...
void* RTalignZero(size_t align, size_t size) {
    if (0 == align) fprintf(stderr, "Zero alignment in RTalignZero");
...
//    void *p = calloc((size / align + 1), align);
    ...
    if (p == MAP_FAILED)
        fprintf(stderr, "mmap failed for size %zu MB", size>>20);
    size_t pp = (size_t)p;
    void *old = p;
...
static FORCE_INLINE inline uint32_t getblock32 ( const uint32_t * p, int i )
{
  return p[i];
}

static FORCE_INLINE inline uint64_t getblock64 ( const uint64_t * p, int i )
{
  return p[i];
}
...
static FORCE_INLINE inline uint32_t fmix32 ( uint32_t h )
{
  h ^= h >> 16;
  h *= 0x85ebca6b;
  h ^= h >> 13;
...
static FORCE_INLINE inline uint64_t fmix64 ( uint64_t k )
{
  k ^= k >> 33;
...
static hti_t *hti_alloc (hashtable_t *parent, size_t scale) {
    ...
    HREassert((((size_t)hti->table) % CACHE_LINE_SIZE) == 0,
              "Table %p not aligned at %d", (void*)hti->table, CACHE_LINE_SIZE); // cache aligned
